...
int main(int argc, char** argv) {
    ...
    static struct option long_options[] = {
        {"tasks", 1, 0, 't'},
        {"view",  1, 0, 'v'},
        {"dir", 1, 0, 'd'},     //@@ Thien Lu added
        {"help",  0, 0, '?'},
        {0 ,0, 0, 0}
    };

    //@@ Thien Lu added
    std::string outdir = "results/sol";     
    int numTask = 1;
    //@@
    while ((opt = getopt_long(argc, argv, "t:v:d:?", long_options, NULL)) != EOF) {
        ...
        switch (opt) {
            ...
            case 'd': 
            {
                std::string arg_dir = optarg;
                // change view settings
                if (arg_dir.empty()) {
                    std::cerr << "Invalid output dir " << arg_dir << " default to" << outdir << std::endl;
                } 
                else {
                    outdir = arg_dir;
                    std::cout << "Output result directory set to " << outdir << std::endl;
                }
                break;
            }
            //@@
    ...
    if (useTasks) {
        ...
        for (int i = 0; i < 3; ++i) {
            double startTime = CycleTimer::currentSeconds();
            mandelbrot_ispc_withtasks(x0, y0, x1, y1, width, height, maxIterations, output_ispc_tasks, numTask);
            double endTime = CycleTimer::currentSeconds();
    ...
    //@@ Thien Lu added
    // compute speedup
    float speedUpTask = minSerial / minTaskISPC;

    // write result 
    std::string speedoutTask = outdir + "/speed_results_task_ispc.txt";
    writeSpeedResults(speedoutTask, numTask, speedUpTask);
    //@@
