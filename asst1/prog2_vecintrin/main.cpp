...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_vec_float result;
  __cs149_mask maskAll, maskZero, maskNotZero, maskClamp;
  ...
  for (i = 0; i + VECTOR_WIDTH < N; i += VECTOR_WIDTH) {
    ...
    // if (y == 0) {
    _cs149_veq_int(maskZero, y, zero, maskAll);   
    ...
    // float result = x;
    _cs149_vload_float(result, values+i, maskNotZero);  

    // int count = y - 1;
    _cs149_vsub_int(count, y, one, maskAll);
    _cs149_vgt_int(maskNotZero, count, zero, maskAll);
    ...
    while (_cs149_cntbits(maskNotZero) > 0) {
      ...
      // count--;
      _cs149_vsub_int(count, count, one, maskNotZero);
      _cs149_vgt_int(maskNotZero, count, zero, maskAll);
    }
...
float arraySumVector(float* values, int N) {
  ...
  _cs149_vstore_float(out, result, maskAll);
...
int main(int argc, char * argv[]) {
  ...
  printf("Start initing value...\n");
  initValue(values, exponents, output, gold, N);

  printf("Start clamped exp serial...\n");
  clampedExpSerial(values, exponents, gold, N);
  printf("Finish clamped exp serial, starting exp vector...\n");
