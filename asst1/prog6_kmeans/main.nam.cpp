...
extern void kMeansThread(double *data, double *clusterCentroids,
                      ...
                      double epsilon);
extern void kMeansSeq(double *data, double *clusterCentroids,
                      int *clusterAssignments, int M, int N, int K,
                      double epsilon);
...
int main() {
  ...
  kMeansSeq(data, clusterCentroids, clusterAssignments, M, N, K, epsilon);
  double endTimeseq = CycleTimer::currentSeconds();
  printf("[Total Time]: %.3f ms\n", (endTimeseq - startTimeseq) * 1000);
  ...
  double startTimethr = CycleTimer::currentSeconds();
  kMeansThread(data, clusterCentroids, clusterAssignments, M, N, K, epsilon);
  double endTimethr = CycleTimer::currentSeconds();
  printf("[Total Time]: %.3f ms\n", (endTimethr - startTimethr) * 1000);

  printf("Speedup: %.3f\n", (endTimeseq - startTimeseq) / (endTimethr - startTimethr));
