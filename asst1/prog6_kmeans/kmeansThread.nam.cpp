 ...
 * Assigns each data point to its "closest" cluster centroid.
 */
void computerAssignmentsThread(WorkerArgs *const args, int start, int k, int numThreads, double *minDist) {
  // std::cout << "Thread " << start << " started" << std::endl;
  for (int i = 0; i < args->M/numThreads; i++) {
    int m = start + i;
...
void computeAssignmentsSpeedup(WorkerArgs *const args) {
  double *minDist = new double[args->M];
  ...
  for (int k = args->start; k < args->end; k++) {
    ...
    for (unsigned int i=1; i<numThreads; i++) {
      workers[i] = std::thread(computerAssignmentsThread, args, i*(args->M/numThreads), k, numThreads, minDist);
    }
    computerAssignmentsThread(args, 0, k, numThreads, minDist);
    for (unsigned int i=1; i<numThreads; i++) {
      workers[i].join();
    }
  }
...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  int iter = 0;
  double timeAssignments = 0;
  double timeCentroids = 0;
  double timeCost = 0;
  ...
  while (!stoppingConditionMet(prevCost, currCost, epsilon, K)) {
    ...
    computeAssignmentsSpeedup(&args);
    double end = CycleTimer::currentSeconds();
    timeAssignments += (end - start) * 1000;

    start = CycleTimer::currentSeconds();
    computeCentroids(&args);
    end = CycleTimer::currentSeconds();
    timeCentroids += (end - start) * 1000;

    start = CycleTimer::currentSeconds();
    computeCost(&args);
    end = CycleTimer::currentSeconds();
    timeCost += (end - start) * 1000;

    iter++;
  }

  printf("Time Assignments: %.3f ms\n", timeAssignments/iter);
  printf("Time Centroids: %.3f ms\n", timeCentroids/iter);
...
void kMeansSeq(double *data, double *clusterCentroids, int *clusterAssignments,
               int M, int N, int K, double epsilon) {
